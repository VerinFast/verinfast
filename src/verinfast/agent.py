    ...
    def parseRepo(self, path: str, repo_name: str):
        ...
        if not self.config.dry:
            ...
            sizes = {
                ...
                "metadata": {"env": machine, "real_size": real_size, "uname": system},
            }
        ...
        if self.config.runStats:
            ...
            stats_error_file = os.path.join(
                self.config.output_dir, repo_name + ".stats.err"
            )
            ...
            if not self.config.dry:
                ...
                with open(stats_output_file, "w") as f:
                    with open(stats_error_file, "w") as e:
                        subprocess.check_call(
                            ["modernmetric", f"--file={stats_input_file}"],
                            stdout=f,
                            stderr=e,
                            encoding="utf-8",
                        )
        ...
        if self.config.runScan:
            ...
            if self.checkDependency("semgrep", "Semgrep"):
                findings_output_file = os.path.join(
                    self.config.output_dir, repo_name + ".findings.json"
                )
                findings_error_file = os.path.join(
                    self.config.output_dir, repo_name + ".findings.err"
                )
                if not self.config.dry:
                    ...
                    try:
                        ...
                        output = e.output
                        self.log(
                            msg=output, tag="Scanning repository return", display=True
                        )
                try:
                    ...
                    if self.config.truncate_findings:
                        ...
                        self.log(
                            tag="TRUNCATING", msg=f"excluding: {truncation_exclusion}"
                        )
                        try:
                            ...
                            self.log(e)
                            self.log(
                                json.dumps(original_findings, indent=4, sort_keys=True)
                            )
                ...
                self.upload(
                    file=findings_output_file, route="findings", source=repo_name
                )

        # ##### Scan Dependencies ######
        if self.config.runDependencies:
            dependencies_output_file = os.path.join(
                self.config.output_dir, repo_name + ".dependencies.json"
            )
        ...
        for provider in cloud_config:
            try:
                ...
                if provider.provider == "aws" and self.checkDependency(
                    ...
                    self.upload(file=aws_instance_file, route="instances", source="AWS")
                ...
                if provider.provider == "azure" and self.checkDependency(
                    ...
                    azure_utilization_file = (
                        azure_instance_file[:-5] + "-utilization.json"
                    )
                    self.upload(
                        file=azure_utilization_file, route="utilization", source="AWS"
                    )
                    ...
                    self.upload(file=azure_block_file, route="storage", source="Azure")
                ...
                if provider.provider == "gcp" and self.checkDependency(
                    ...
                    self.upload(file=gcp_instance_file, route="instances", source="GCP")
...
def main():
    ...
    if agent.config.shouldUpload or agent.config.upload_logs:
        agent.upload(
            route="logs",
            file=agent.config.output_dir + "/log.txt",
            source="logs",
            isJSON=False,
        )
    if agent.config.upload_logs:
        ...
        print(
            f"""The log for this run has moved to:
