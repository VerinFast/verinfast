...
class RepositoryScanner(GitUtils, RepoHandlers, ScannerTools, FileUtils):
    ...
    def _parse_repo(self, path, repo_name, branch=None):
        ...
        git_output_file = os.path.join(self.config.output_dir, f"{repo_name}.git.log.json")
    ...
    def _run_file_size_scan(self, path, repo_name):
        ...
        sizes_output_file = os.path.join(self.agent.config.output_dir, f"{repo_name}.sizes.json")
        ...
        if not self.config.dry:
            ...
            self._update_template_definition("current_dir_size", sizes["files"].pop("."))
