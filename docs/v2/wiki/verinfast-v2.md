...
| 2 | Dependency keep/replace + license confirmation | [[Dependency & License Review]] — stub |
| 3 | Re-architect the folder structure to current standards | not started |
| 4 | Rebuild against `atd_v3` in good-place (matching API signatures) | contract captured: [[ATD v3 Upload Contract]] |
| 5 | Importable as a Python module so ATD v3 can scan on demand | requirements drafted: [[Requirements: Embeddable Library API]] |
...
- [[Open Questions]] — decisions needed before workstream 3 starts
- [[v2 Workstreams]] — sequencing

## Source of truth

Everything here was derived by reading `VerinFast/verinfast@ea5ad24` (the `main`
tip) and `VerinFast/good-place@74e41ff` (`services/atd`). Where the two
disagree, good-place is the customer and wins — see
